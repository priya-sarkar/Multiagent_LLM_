...
class ScalarDebate(Template):
    ...
    def __init__(self, args, connectivity_matrix):
        ...
        self._direction = [ [1, 1] for _ in range(self._n_agents) ]  # For each agent
        self._PGM = np.ones((self._n_agents, 2))
    ...
    def _generate_agents(self, simulation_ind):
        ...
        """
        names = [f"Agent{i+1}" for i in range(self._n_agents)]
    ...
    def _generate_question(self, agent, round, simulation_ind):
        ...
        agent_index = int(agent.name[-1]) if agent.name[-1].isdigit() else None
        if agent_index == 2:
            left_prob, right_prob = self._PGM[0]  # agent 1 is index 0
            if(left_prob>=right_prob):
                pgm_hint = f"\n It is likely that the other agent is moving to 0 to defuse the bomb."
            else:
                pgm_hint = f"\n It is likely that the other agent is moving to 100 to defuse the bomb."
        else:
            pgm_hint = ""
        if len(bombs) == 0:
        ...
        if round == 0:
            ...
            input = self._round_description.format(bombs_str, agent.position, agent.other_position)+ pgm_hint
        return input
    ...
    def _update_metrics(self, simulation_ind, round, agents, old_bomb_list):
        ...
        if bombs_before == 2 and len(bombs) == 1 and self._metrics[simulation_ind]['first_defuse'] is None:
            self._metrics[simulation_ind]['first_defuse'] = round+1
        if bombs_before == 1 and len(bombs) == 0 and self._metrics[simulation_ind]['second_defuse'] is None:
            self._metrics[simulation_ind]['second_defuse'] = round+1
        if bombs:
            for idx, agent in enumerate(agents):
    ...
    # --- New function to average metrics over all simulations ---
    ...
    #         avg_metrics['rounds_to_clear_bombs'] += metric['rounds_to_clear_bombs']
    #         avg_metrics['first_defuse'] += metric['first_defuse']
    #         avg_metrics['second_defuse'] += metric['second_defuse']
    ...
    #         avg_metrics[key] = round(avg_metrics[key] / total_sims, 2)
